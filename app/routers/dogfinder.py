...
class QueryRequest(BaseModel):
    ...
    top: int = 10
    return_properties: Optional[List[str]] = ["type", "breed", "filename", "image", IS_FOUND_FIELD, DOG_ID_FIELD, "phoneNumber"]
...
async def add_document(type: str = Form(...), breed: Optional[str] = Form(None), img: UploadFile = File(...), phoneNumber: str = Form(...)):
    ...
    try:
        ...
        # Create DogDocument
        dogDocument = DogDocument(type=type, breed=breed, image=img_base64, filename=img.filename,phoneNumber=phoneNumber)
